...
```tsx
import { ListItem, Tag } from '@ledgerhq/ldls-ui-react';
import { Bolt, ChevronRight } from '@ledgerhq/ldls-ui-react/symbols';
...
```tsx
import { ListItem, Switch } from '@ledgerhq/ldls-ui-react';
...
<div className='flex flex-col gap-24'>
  <DoVsDontRow>
    <DoBlockItem
      ...
      description='Layout constraints and proper list grouping are allowed'
    >

```tsx
<ListItem title='Item' className='max-w-md' />
```
...
{/* prettier-ignore */}
```tsx
<ListItem title='Custom' className='bg-base text-active' />
```
...
```
  ...
  <DoVsDontRow>
    <DoBlockItem
      ...
      description='Use specific icons for better performance'
    >

```tsx
import { Check } from '@ledgerhq/ldls-ui-react/symbols';

<ListItem trailingContent={<Check />} />;
```

    </DoBlockItem>
    <DontBlockItem
      title="Don't import all icons"
      description='Avoid importing entire icon set or complex inline handlers'
    >
...
<ListItem trailingContent={<AllIcons.Check />} />
```
