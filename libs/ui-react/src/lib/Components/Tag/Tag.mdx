import { Meta, Canvas, Controls } from '@storybook/addon-docs/blocks';
import * as TagStories from './Tag.stories';
import { CustomTabs, Tab } from '../../../../.storybook/CustomTabs';
import { DoVsDontRow } from '../../../../.storybook/DoVsDontRow';
import { DoBlockItem } from '../../../../.storybook/DoBlock';
import { DontBlockItem } from '../../../../.storybook/DontBlock';

<Meta title='Components/Tag' of={TagStories} />

# 🏷️ Tag

<CustomTabs>
  <Tab label="Overview">

View in Figma: [link](https://www.figma.com/design/JxaLVMTWirCpU0rsbZ30k7/2.-Components-Library?node-id=722-5547&m=dev)

## Anatomy

- **Label:** The text content of the tag.
- **Icon (optional):** An optional icon displayed before the label.

## Properties

### Overview

<Canvas of={TagStories.Default} />
<Controls of={TagStories.Default} />

### Appearance

<Canvas of={TagStories.Showcase} />

---

### Size

Tags come in two different sizes:

- **lg** (default)
- **sm**

<Canvas of={TagStories.SizesShowcase} />

### Icon

Tags can be displayed with or without an icon.

<Canvas of={TagStories.ContentTypesShowcase} />

### Accessibility

To be implemented:

- **Color contrast**
- **Text zoom**
- **Keyboard navigation**
- **Semantic labeling**

</Tab>
<Tab label="Implementation">

```bash
npm install @ledgerhq/ldls-ui-react
```

> **Note**: `@ledgerhq/ldls-design-core` and other peer dependencies (`react`, `tailwindcss`, `clsx`, etc.) are required. See our [Setup Guide →](?path=/docs/getting-started-setup-tailwind--docs#1-tailwind-config) for complete installation and Tailwind configuration.

## Basic Usage

```tsx
import { Tag } from '@ledgerhq/ldls-ui-react';

function MyComponent() {
  return <Tag size='lg' appearance='accent' label='My Tag' />;
}
```

### Custom Styling

While the component comes with predefined styles, you can extend them using Tailwind classes:

```tsx
<Tag
  size='lg'
  appearance='accent'
  label='Custom Tag'
  className='mt-4' // Custom margin
/>
```

### With Icons

The Tag component supports icons with automatic size adjustment based on the tag size:

```tsx
import { Tag } from '@ledgerhq/ldls-ui-react';
import { Check } from '@ledgerhq/ldls-ui-react/symbols';

function MyComponent() {
  return <Tag size='lg' appearance='success' icon={Check} label='Success' />;
}
```

## Do's and Don'ts

<div className='flex flex-col gap-24'>
  <DoVsDontRow>
    <DoBlockItem
      title='Use label for text content'
      description='Pass icon as a component reference for proper sizing'
    >

```tsx
import { Plus } from '@ledgerhq/ldls-ui-react/symbols';
<Tag icon={Plus} label='Add Item' />
```

    </DoBlockItem>
    <DontBlockItem
      title="Don't include icon in label"
      description='Avoid including icon elements directly in label text'
    >

{/* prettier-ignore */}
```tsx
<Tag label='<Plus /> Add Item' />
```

    </DontBlockItem>

  </DoVsDontRow>

  <DoVsDontRow>
    <DoBlockItem
      title='Use className for layout'
      description='Layout positioning and responsive behavior are allowed'
    >

```tsx
<Tag className='mt-4 ml-2' label='Submit' />
```

    </DoBlockItem>
    <DontBlockItem
      title="Don't override core styling"
      description="Core styling, spacing, and typography should not be modified"
    >

{/* prettier-ignore */}
```tsx
<Tag className='bg-black' label='Custom Background' />
```

    </DontBlockItem>

  </DoVsDontRow>

  <DoVsDontRow>
    <DoBlockItem
      title='Import icons explicitly'
      description='Use specific imports for better tree-shaking'
    >

```tsx
import { Check } from '@ledgerhq/ldls-ui-react/symbols';

<Tag icon={Check} label='Success' />
```

    </DoBlockItem>
    <DontBlockItem
      title="Don't import all icons"
      description='Avoid importing entire icon library or using string-based names'
    >

{/* prettier-ignore */}
```tsx
import * as Icons from '@ledgerhq/ldls-ui-react/symbols';
```

    </DontBlockItem>

  </DoVsDontRow>
</div>

> **Note**: Use `className` only for layout-related utilities (positioning, margins, display properties). The tag's core styling (colors, padding, etc.) and typography should be controlled through the component's props (`appearance`, `size`) to maintain design system consistency.

</Tab>
</CustomTabs>
