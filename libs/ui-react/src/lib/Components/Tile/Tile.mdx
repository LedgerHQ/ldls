...
Tiles come in two sizes:

- **md** (medium, default): Standard tile size with comfortable padding and body-2-semi-bold typography
- **sm** (small): Compact tile with reduced padding and body-3-semi-bold typography

<Canvas of={TileStories.SizeShowcase} />

> **Note**: Use a `32` Spot for small size tiles.
...
<Tile
  title='Settings'
  description='Configure app'
  leadingContent={<Spot appearance='icon' icon={Settings} size='sm' />}
  size='sm'
/>
```
...
```
  ...
  <DoVsDontRow>
    <DoBlockItem
      title='Use size 32 for Spot when Tile size is small'
      description='When using a small Tile, reduce the Spot size to 32 to maintain proper proportions'
    >
...
```
    ...
    <DontBlockItem
      title="Don't use default Spot size with small Tile"
      description='Using the default Spot size (48) with a small Tile creates poor visual proportions'
    >
