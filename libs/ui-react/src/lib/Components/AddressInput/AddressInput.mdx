...
{/* prettier-ignore */}
```tsx
// DON'T - prefix is a string, not a ReactNode
<AddressInput 
...
<AddressInput
  value={address}
  onChange={handleChange}
  suffix={<Button onClick={() => setAddress('')}>Clear</Button>}
/>
```
...
```
    ...
    <DontBlockItem
      ...
      description='These props are not available on AddressInput. Use className for container-level styling only'
    >

{/* prettier-ignore */}
```tsx
// These props don't exist on AddressInput
<AddressInput 
