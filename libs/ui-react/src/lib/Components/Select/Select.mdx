...
<div className='flex flex-col gap-24'>
  <DoVsDontRow>
    <DoBlockItem
      ...
      description='Layout positioning and responsive behavior are allowed on SelectTrigger and SelectContent'
    >

{/* prettier-ignore */}
```tsx
<Select value={value} onValueChange={setValue}>
  <SelectTrigger label='Category' className='max-w-md' />
...
```
    ...
    <DontBlockItem
      ...
      description='Core styling, internal spacing, and typography should not be modified by className'
    >

{/* prettier-ignore */}
```tsx
<Select value={value} onValueChange={setValue}>
  <SelectTrigger label='Category' className='bg-base text-active' />
  <SelectContent className='bg-base text-active'>
    <SelectItem value='option1'>
      <SelectItemText>Option 1</SelectItemText>
    </SelectItem>
  </SelectContent>
