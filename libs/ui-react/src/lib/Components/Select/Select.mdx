...
#### **About @radix-ui/react-select**

The `@radix-ui/react-select` dependency is used internally by the Select component
to provide accessible, unstyled select functionality with proper keyboard navigation,
focus management, and screen reader support. It handles the core select behavior including
open/closed states, option selection, and accessibility features, while our component adds the visual styling and design system tokens.
