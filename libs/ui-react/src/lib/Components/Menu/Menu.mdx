...
<div className='flex flex-col gap-24'>
  <DoVsDontRow>
    <DoBlockItem
      ...
      description='Menu items should clearly describe what will happen when selected'
    >
      ```tsx
      {/* prettier-ignore */}
      <Menu>
    ...
    <DontBlockItem
      title="Don't use vague or unclear labels"
      description='Avoid ambiguous text that leaves users uncertain'
    >
      ```tsx
      {/* prettier-ignore */}
      <Menu>
...
<DoVsDontRow>
  ...
  >
    ```tsx
    {/* prettier-ignore */}
    <MenuItem onClick={() => setShowPanel(!showPanel)}>
        Show Panel
      </MenuItem>
    ```
  </DontBlockItem>
...
<DoVsDontRow>
  ...
  >
    ```tsx
    {/* prettier-ignore */}
    <MenuCheckboxItem checked={theme === 'light'}>
        Light
      </MenuCheckboxItem>
