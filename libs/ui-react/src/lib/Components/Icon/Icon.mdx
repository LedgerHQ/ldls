import {
  Meta,
  Canvas,
  Controls,
  IconGallery,
  IconItem,
} from '@storybook/addon-docs/blocks';
import * as Icons from '../../Symbols';
import * as IconsStories from './Icon.stories';
import {
  CustomTabs,
  Tab,
  DoVsDontRow,
  DoBlockItem,
  DontBlockItem,
} from '../../../../.storybook/components';
import CommonRulesDoAndDont from '../../../../.storybook/components/DoVsDont/CommonRulesDoAndDont.mdx';

<Meta title='Symbols/Interface Icons/Docs' of={IconsStories} />

# 🎨 Icons

<CustomTabs>
  <Tab label="Overview">

Icons are essential elements that communicate meaning quickly and visually. They support navigation, clarify actions, and enhance the overall user experience. A well-designed icon set ensures clarity, consistency, and scalability across platforms.

---

## Figma Library

All icons are designed and managed in our Figma Foundations library. This is the single source of truth for our icon set.

- 🔗 **[Figma - Icons Library](https://www.figma.com/design/zSkvGGiqcnhywp2l3HTHxA/1.Symbol-Library?node-id=50-5110&m=dev)**

---

## Interactive Example

<Canvas of={IconsStories.Icon} />
<Controls of={IconsStories.Icon} />

## 📏 Sizing

Our icons are designed to be scalable and are available in 7 standard sizes to fit various contexts within the UI.

The available sizes are: **16px, 20px, 24px, 32px, 40px, 48px, and 56px**

The thickness of each icon is automatically managed by our token system when you apply a size variable, ensuring visual consistency at any scale.

<Canvas of={IconsStories.IconSizes} />

---

## Icon Gallery

Here's a preview of all the icons available in our design system. To use an icon, just reference its name.

<Canvas of={IconsStories.Gallery} />

</Tab>
<Tab label="Implementation">

```bash
npm install @ledgerhq/lumen-ui-react
```

> **Note**: `@ledgerhq/lumen-design-core` and other peer dependencies (`react`, `tailwindcss`, `clsx`, etc.) are required. See our [Setup Guide →](?path=/docs/getting-started-setup-tailwind--docs#1-tailwind-config) for complete installation and Tailwind configuration.

## Basic Usage

Import and use specific icons from the symbols library:

```tsx
import { ArrowUp, Settings, Plus } from '@ledgerhq/lumen-ui-react/symbols';

function MyComponent() {
  return (
    <div>
      <ArrowUp size={24} />
      <Settings size={20} />
      <Plus size={16} />
    </div>
  );
}
```

### Size Variants

Icons support predefined size variants that ensure consistent visual hierarchy:

```tsx
import { ArrowUp } from '@ledgerhq/lumen-ui-react/symbols';

function IconSizes() {
  return (
    <div className='flex items-center gap-4'>
      <ArrowUp size={16} />
      <ArrowUp size={20} />
      <ArrowUp size={24} /> {/* Default size */}
      <ArrowUp size={40} />
      <ArrowUp size={48} />
      <ArrowUp size={56} />
    </div>
  );
}
```

### Integration with Buttons

Icons are seamlessly integrated into our button components to ensure consistent sizing and alignment.

**Button with Icon**

Use the [Button](?path=/docs/components-button--docs) component to combine a label with an icon. The icon size is automatically adjusted based on the button size.

```tsx
import { Button } from '@ledgerhq/lumen-ui-react';
import { ArrowRight } from '@ledgerhq/lumen-ui-react/symbols';

function MyComponent() {
  return (
    <Button size='m' appearance='base' icon={ArrowRight}>
      Continue
    </Button>
  );
}
```

**Icon Only**

For Icon-only buttons, use the [IconButton](?path=/docs/action-iconbutton--docs) component.

```tsx
import { IconButton } from '@ledgerhq/lumen-ui-react';
import { Settings } from '@ledgerhq/lumen-ui-react/symbols';
<IconButton icon={Settings} />;
```

### Accessibility

Always provide proper accessibility attributes for interactive icons:

```tsx
import { Information } from '@ledgerhq/lumen-ui-react/symbols';

function AccessibleIcon() {
  return <Information size={20} aria-label='Account verification required' />;
}
```

### With Semantic Colors

Icons inherit the current text color and can be styled with semantic color utilities:

```tsx
import {
  CheckmarkCircle,
  DeleteCircle,
  Clock,
} from '@ledgerhq/lumen-ui-react/symbols';

function SemanticIcons() {
  return (
    <div className='flex gap-4'>
      <CheckmarkCircle size={24} className='text-success' />
      <DeleteCircle size={24} className='text-error' />
      <Clock size={24} className='text-warning' />
    </div>
  );
}
```

## Do's and Don'ts

<div className='flex flex-col gap-24'>

  <DoVsDontRow>
    <DoBlockItem
      title='Use predefined size variants'
      description='Standard sizes ensure consistent visual hierarchy'
    >

```tsx
<ArrowUp size={24} />
```

    </DoBlockItem>
    <DontBlockItem
      title="Don't use custom sizes"
      description='Avoid CSS sizing, prop overrides, or non-standard sizes'
    >

{/* prettier-ignore */}
```tsx
<ArrowUp className='w-6 h-6' />
```

    </DontBlockItem>

  </DoVsDontRow>

  <DoVsDontRow>
    <DoBlockItem
      title='Use semantic colors for icons'
      description='Use semantic color tokens and inherit text color'
    >

```tsx
<ArrowUp className='text-success' />
```

    </DoBlockItem>
    <DontBlockItem
      title="Don't use arbitrary colors"
      description='Don not use arbitrary colors or overriding currentColor unnecessarily'
    >

{/* prettier-ignore */}
```tsx
<ArrowUp className='text-[red]' />
```

    </DontBlockItem>

  </DoVsDontRow>

  <CommonRulesDoAndDont />
</div>

</Tab>
</CustomTabs>
