...
- **Actions (optional)**: Primary and/or secondary buttons for user interaction. Use Banner.PrimaryAction and Banner.SecondaryAction slots with Button components.
- **Close (optional)**: Button to dismiss the banner. Use Banner.CloseAction slot with a Button containing the Close icon.
...
> **Note**: The title is required. Description, actions, and close are optional and can be combined as needed. Primary, secondary and close actions need to be provided as children using the corresponding slot components.
...
```tsx
// Use slot components with Button elements
<Banner
...
// Combine multiple slot components
<Banner
  title="Confirm Action"
  description="This action cannot be undone"
  appearance="warning"
...
```tsx
// Don't provide arbitrary non-slot children
<Banner
  title="Incomplete Action"
  description="Missing button text or handler"
  appearance="info"
>
  <Button appearance="transparent" size="sm">
    Primary action
