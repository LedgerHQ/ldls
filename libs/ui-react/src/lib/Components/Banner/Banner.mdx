...
> View in [Figma](https://www.figma.com/design/JxaLVMTWirCpU0rsbZ30k7/2.-Components-Library?m=dev).
...
- **Actions (optional)**: Primary and/or secondary buttons for user interaction. Provide a label and a click handler for an action to be displayed (eg. `primaryActionLabel` and `onPrimaryActionClick`).
- **Close (optional)**: Button to dismiss the banner. Provide the `onCloseClick` handler for the close button to be displayed.
