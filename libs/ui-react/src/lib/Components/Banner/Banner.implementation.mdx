...
function ActionableBanner() {
  ...
  return (
    <Banner
      ...
      onPrimaryActionClick={handleAccept}
      onSecondaryActionClick={handleDecline}
    />
  );
}
```
    ...
    <div className="space-y-4">
      {notifications.map(notification => (
        <Banner
          ...
          isFull
          onCloseClick={notification.dismissible ? () => dismissNotification(notification.id) : undefined}
          primaryActionLabel={notification.actions ? "Verify Now" : undefined}
          secondaryActionLabel={notification.actions ? "Later" : undefined}
          onPrimaryActionClick={notification.actions ? handleVerify : undefined}
          onSecondaryActionClick={notification.actions ? handleLater : undefined}
        />
      ))}
    </div>
  );
