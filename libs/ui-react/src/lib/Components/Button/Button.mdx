...
import { DoBlock, DoBlockItem } from '../../../../.storybook/DoBlock';
import { DontBlock, DontBlockItem } from '../../../../.storybook/DontBlock';
...
```tsx
<Button icon={Plus} size='md' className='w-6 h-6'>
  Add Item
</Button>
```
...
```
  ...
  <DoBlockItem
    title='Use className for responsive behavior'
    description='Control visibility and layout at different breakpoints'
  >
...
```tsx
<Button className='bg-red-500'>Wrong Background</Button>
```

  </DontBlockItem>

  <DontBlockItem
    title="Don't modify internal spacing"
    description='Padding and gaps are managed by the size prop'
  >

```tsx
<Button className='p-8 gap-8'>Wrong Spacing</Button>
```
...
```
  ...
  <DontBlockItem
    title="Don't override typography"
    description='Font size is controlled by the size prop'
  >
