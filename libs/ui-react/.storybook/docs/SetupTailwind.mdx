...
Install `ldls-design-core` and `ldls-ui-react` packages:
...
3. **Cursor AI Rules** (Optional)
   - For Cursor users, LDLS includes AI rules for better code suggestions
   - Copy rules after installing the package:
     ```bash
     cp -r node_modules/@ledgerhq/ldls-ui-react/cursor-rules/* .cursor/rules/
     ```
   - [View documentation →](https://www.npmjs.com/package/@ledgerhq/ldls-ui-react#-using-with-ai-assistants-cursor)
...
### 5. Avoid Overriding Preset Configurations
...
## 🤖 AI Assistant Support (Cursor)

Working with Cursor AI? LDLS includes rules to help AI assistants suggest correct patterns:

```bash
# Copy after installing @ledgerhq/ldls-ui-react
cp -r node_modules/@ledgerhq/ldls-ui-react/cursor-rules/* .cursor/rules/
```
...
[View cursor rules →](https://www.npmjs.com/package/@ledgerhq/ldls-ui-react#-using-with-ai-assistants-cursor) | [Cursor Documentation](https://docs.cursor.com/context/rules-for-ai)
