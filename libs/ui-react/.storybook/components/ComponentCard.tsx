  ...
  return (
    ...
    >
      ...
      <div className='flex flex-1 flex-col items-center justify-between'>
        <div>
          <h4 className='!mb-0 block !cursor-pointer text-base heading-5 group-hover:text-[#B380DD] md:hidden'>
            {emoji && <div>{emoji}</div>}
            {title}
          </h4>
          <h3 className='!mb-0 hidden !cursor-pointer text-base heading-5 group-hover:text-[#B380DD] md:block'>
            {emoji && <div>{emoji}</div>}
            {title}
          </h3>
