...
```tsx
import { LumenStyleSheet } from '@ledgerhq/ldls-ui-rnative';
...
const Button = ({ appearance, disabled }) => {
  const styles = LumenStyleSheet.useCreate((theme) => {
    const getBackgroundColor = () => {
      ...
      if (variant === 'primary') return theme.colors.bg.accent;
    ...
    return {
      ...
      text: {
        ...
        color: appearance === 'accent' ? theme.colors.text.onAccent : theme.colors.text.base,
      },
    };
  });
