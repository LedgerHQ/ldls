import { Meta, Canvas, Controls } from '@storybook/addon-docs/blocks';
import * as BoxStories from './Box.stories';

<Meta title='Layout/Box' of={BoxStories} />

# Box

Box is a layout primitive that wraps React Native's `View` component with token-constrained style props from the design system theme.

## Interactive Example

<Canvas of={BoxStories.Base} />
<Controls of={BoxStories.Base} />

## Usage

```tsx
import { Box, Text } from '@ledgerhq/ldls-ui-rnative';

function Card() {
  return (
    <Box
      padding='s16'
      backgroundColor='accent'
      borderRadius='md'
      gap='s8'
    >
      <Text>Card content</Text>
    </Box>
  );
}
```

## Theme Tokens

Box consumes tokens from the `ledgerLiveTheme` for spacing, sizes, colors, and border radius.

## Escape Hatch

For values not covered by tokens, use raw numbers or the `style` prop:

```tsx
// Raw number for one-off values
<Box width={127} marginTop='s4' />

// Style prop for complete override
<Box marginTop='s4' style={{ width: 127 }} />
```
