...
The CardButton component is built on TouchableOpacity and provides:

- **Touch feedback**: Visual indication when pressed through opacity changes
- **activeOpacity**: Set to 1 for consistent behavior across the design system
...
npm install react-native@^0.73.0 react@^18.3.1 nativewind@^4.1.0 tailwindcss@^3.4.0 class-variance-authority
```

### NativeWind Configuration

To use the CardButton component with our custom NativeWind setup, you need to configure your `tailwind.config.ts`:
