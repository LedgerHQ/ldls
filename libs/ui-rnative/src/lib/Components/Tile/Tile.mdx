...
<Box lx={{ flexDirection: 'column', gap: 's24' }}>
  <DoVsDontRow>
    <DoBlockItem
      title='Use Box wrapper to control width'
      description='Wrap Tile in a Box component with lx props to control its width'
    >
...
```
    ...
    <DontBlockItem
      title="Don't use lx for width control on Tile"
      description='Avoid using lx directly on Tile for layout control'
    >
