...
> **Important:** TabBar requires a minimum of **2 TabBarItem children**. Using fewer than 2 items will result in a TypeScript error. This ensures a meaningful navigation experience for users.
...
- Each tab item includes proper `accessibilityRole="tab"` with `accessibilityLabel` for screen readers
- Active state is communicated through `accessibilityState={{ selected: true }}` for assistive technologies
- The TabBar container uses `accessibilityRole="tablist"` to semantically group tab items
- Labels automatically fallback to `value` prop ensuring all tabs have accessible names
- Each tab item is fully pressable with appropriate touch targets
- Active state is visually indicated through bold text and icon changes
- Smooth animations provide visual feedback for user interactions
