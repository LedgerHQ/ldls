...
<Meta title='Input/Select' of={SelectStories} />
...
<Canvas of={SelectStories.WebPreviewWarning} />

## Anatomy

> **Layout:** The Select component takes the full width of its container by default. You can control the width directly using `className` or by wrapping in a container.
...
The bottom sheet is managed globally through:

- **GlobalSelectProvider**: Manages the global state of select bottom sheets
- **GlobalSelectBottomSheet**: The actual bottom sheet component that displays options

These must be set up at your app's root level (inside `GestureHandlerRootView`).
...
Before using Select components, you must set up the global providers at your app's root:
  ...
  return (
    <GestureHandlerRootView style={{ flex: 1 }}>
      <GlobalSelectProvider>
        {/* Your app content */}
        <YourAppContent />
        {/* Bottom sheet should be at root level */}
        <GlobalSelectBottomSheet />
      </GlobalSelectProvider>
...
> **Note:** Both `GlobalSelectProvider` and `GlobalSelectBottomSheet` are required for the Select component to work properly.
...
```tsx
<Select value={value} onValueChange={setValue}>
  <SelectTrigger label='Choose an option' className='max-w-md'>
    <SelectValue className='heading-0' />
  </SelectTrigger>
  <SelectContent>
    <SelectItem value='option1'>
      <SelectItemText>Option 1</SelectItemText>
...
The Select component takes the **full width** of its parent container by default. You can control the width using `className` (preferred) or wrapper containers:
...
<Select value={value} onValueChange={setValue}>
  <SelectTrigger label='Option' className='max-w-md'>
    <SelectValue />
  </SelectTrigger>
  <SelectContent>
    <SelectItem value='option1'>
...
<Select value={value} onValueChange={setValue}>
  <SelectTrigger label='Option' className='mt-4 mx-auto'>
    <SelectValue />
  </SelectTrigger>
  <SelectContent>
    <SelectItem value='option1'>
...
```tsx
// Don't use inline styles when className is available
<SelectTrigger label='Option' style={{ maxWidth: 300 }}>
  <SelectValue />
</SelectTrigger>
```
...
// Use labelClassName for label styling
<Select value={value} onValueChange={setValue}>
  <SelectTrigger label='Important' labelClassName='text-accent'>
    <SelectValue />
  </SelectTrigger>
  <SelectContent>
    <SelectItem value='option1'>
...
// Don't use className to style labels (use labelClassName instead)
<SelectTrigger label='Option' className='text-accent'>
  <SelectValue />
</SelectTrigger>
```
