...
- Uses `accessibilityRole='button'` for proper screen reader support
...
```
    ...
    <DontBlockItem
      title="Don't use style for width"
...
{/* prettier-ignore */}
```tsx
<TileButton icon={Settings} onPress={() => navigation.navigate('Settings')}>
  Settings
</TileButton>
```
