...
> ⚠️ **Unsupported previews on web**
>
> For this component, previewing stories is currently only supported on mobile. Reanimated causes compatibility issues with the web preview, so for now, **please test on-device**.
> We plan to add screenshots or embed Expo Snack in the future.

## Anatomy

> **Layout:** The Search component takes the full width of its container by default. You can control the width directly using `className` or by wrapping in a container.
...
```tsx
import { Search } from '@ledgerhq/ldls-ui-rnative';
...
```tsx
// Use controlled input pattern (recommended)
const [query, setQuery] = useState('');
<Search
  placeholder='Search'
  value={query}
...
<Search
  ...
  value={query}
  onChangeText={setQuery}
  onClear={() => setQuery('')} // Redundant - clearing happens by default
/>
...
<Search
  placeholder='Search'
  value={query}
  onChangeText={(text) => performExpensiveSearch(text)} // Bad performance
/>

// Don't use labels with Search
<Search label='Search' placeholder='Search products' /> // label is not supported

// Don't override the built-in search icon
<Search prefix={<CustomIcon />} /> // prefix is not customizable
```
...
<Search placeholder='Search' className='mt-4 mx-auto' />
...
```tsx
// Always provide clear, descriptive placeholder text
<Search
...
// Use appropriate keyboard types when needed
<Search
  placeholder='Search by email'
  value={query}
  onChangeText={setQuery}
...
// Provide error messages when search fails
<Search
  placeholder='Search products'
  value={query}
  onChangeText={setQuery}
...
<Search placeholder='Type here' value={query} onChangeText={setQuery} />
...
<Search placeholder='Search' value={query} onChangeText={handleChange} />;
...
<Search
  ...
  onChangeText={(text) => {
    setQuery(text);
    performExpensiveAPICall(text); // Bad - should be debounced
  }}
/>
```
