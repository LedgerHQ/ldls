...
it('tokenFileNameRenamer on breakpoints collection', async () => {
...
it('tokenFileNameRenamer on brand collection', async () => {
  ...
  expect(tokenFileNameRenamer(brandCollection, brandCollection.modes[0])).toBe(
    '3.Brand.LedgerLive.json'
  );
  expect(tokenFileNameRenamer(brandCollection, brandCollection.modes[1])).toBe(
